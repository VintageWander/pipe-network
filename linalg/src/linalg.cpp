...
namespace cie::linalg {
    namespace linalghelper {
        ...
        void
        runtime_check( bool result, const char message[] ) {
            if ( !result ) {
    ...
    Matrix::Matrix( size_t size1, size_t size2, double value ) : size1_( size1 ), size2_( size2 ), data_( size1 * size2, value ) {
    }
    ...
    Matrix::Matrix( const std::vector<double>& rowMajorData, size_t size1 ) : size1_( size1 ), size2_( 0 ), data_( rowMajorData ) {
        if ( size1 != 0 ) {
    ...
    namespace linalghelper {
        ...
        void
        writeRow( const VectorType& vector, size_t size, std::ostream& out, size_t digits ) {
    ...
    void
    write( const Vector& vector, std::ostream& out ) {
        linalghelper::writeRow( [ & ]( size_t i ) { return vector[ i ]; }, vector.size(), out, 12 );
    }

    void
    write( const Matrix& matrix, std::ostream& out ) {
    ...
    namespace linalghelper {
        ...
        void
        updatePermutation( const Matrix& matrix,
